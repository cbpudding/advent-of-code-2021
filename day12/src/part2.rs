...
fn travel(caves: &HashMap<String, Cave>, cave: &Cave, hstry: Vec<String>, extra: bool) -> usize {
    ...
    for c in connected {
        ...
        if c == "end" {
            ...
            if !history.contains(c) || subcave.revisitable() {
                history.push(c.clone());
                paths += travel(caves, subcave, history.clone(), extra);
            } else if extra && c != "start" {
                history.push(c.clone());
                paths += travel(caves, subcave, history.clone(), false);
            }
        }
    }
    paths
...
fn main() {
    ...
    if args.len() > 1 {
        ...
        let paths = travel(&caves, start, vec![String::from("start")], true);
